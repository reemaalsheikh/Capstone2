...
public class AppointmentsController {
    ...
    //add new appointment
    @PostMapping("/add")
    public ResponseEntity addNewAppointment(@Valid @RequestBody Appointments appointments, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public  ResponseEntity updateAppointments(@Valid @RequestBody Appointments appointments ,@PathVariable Integer id,Errors errors){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
