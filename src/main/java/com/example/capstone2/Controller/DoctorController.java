...
public class DoctorController {
    ...
    // Add new Doctor
    @PostMapping("/add")
    public ResponseEntity addNewDoctor(@Valid @RequestBody Doctor doctor, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public  ResponseEntity updateDoctor(@PathVariable Integer doctorId,@Valid @RequestBody Doctor doctor , Errors errors){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
