...
public class CDPredictionController {
 ...
 //Get All Chronic Disease Prediction
    ...
    @PostMapping("/add")
    public ResponseEntity addNewChronicDP(@Valid @RequestBody CDPrediction cdPrediction, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public  ResponseEntity updateChronicDP(@Valid @RequestBody CDPrediction cdPrediction ,@PathVariable Integer id,Errors errors){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
