...
public class HealthDataController {
    ...
    // Add new Health data
    @PostMapping("/add")
    public ResponseEntity addNewHealthData(@Valid @RequestBody HealthData healthData, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public  ResponseEntity updateUser(@PathVariable Integer id,@Valid @RequestBody HealthData healthData , Errors errors){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
