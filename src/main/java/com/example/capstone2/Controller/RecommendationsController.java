...
public class RecommendationsController {
    ...
    @PostMapping("/add")
    public ResponseEntity addNewRecommendations(@Valid @RequestBody Recommendations recommendations, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public  ResponseEntity updateRecommendation(@Valid @RequestBody Recommendations recommendations ,@PathVariable Integer id,Errors errors){
        if (errors.hasErrors()) {
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
